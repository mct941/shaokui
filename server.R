...
library(gtools);library(ggplot2);library(GGally)
library(magrittr);library(stringr);library(lubridate)
library(plyr);library(dplyr);library(tidyr)
library(readr);library(readxl);library(shiny)
library(shinyBS);library(lazyeval)
...
read.nonmem <- function(file, n=-1) {
  my.split <- function(line, numeric=FALSE) {
    ...
    if( numeric ) return(as.numeric(pieces))
    else return(pieces)
  }
  ...
  for( i in 2:nrow(data)) df <- cbind(df, data[i,])
  rownames(df) <- NULL; colnames(df) <- header
  return(df)
}
...
server <- (function(input, output, session) { 
  ...
  if(input$nonmem) dta <- read.nonmem(inFile$datapath) else
    dta <- read.csv(inFile$datapath,header=input$header,sep=input$sep,
                    quote=input$quote,strings=input$strings,na.strings=input$na)
  return(dta)
  })
  # Display data summary (eg number of rows and columns and summary statistics for each variable)
  output$dim <- renderText({if(is.null(input$file)) return(NULL)
    c("File Name:",input$file$name,
      "(","Observations:",dim(inputdta())[1],";",
      "Variables:",as.numeric(dim(inputdta())[2]),")")
  })
  ...
  ## Variables
  # Panel headers
  output$MAP <- renderText("Data Mapping:");output$TRANS <- renderText("Data Transformation:")
  output$SUB <- renderText("Data Subsetting:");output$SORT <- renderText("Data Sorting:")
  
  # Searches default variable names (eg.ID,TIME,DV,and REP); otherwise user-assigned
  ...
  # A unique identifier is created as defined by user
  ...
  # Present examples of unique identifiers being used
  output$UNIQUEID <- renderPrint({if(is.null(input$file)) return(NULL);
    head(unique(as.character(dta_unique()$UNIQUE)),20)
  })
  # Select variables to convert between continuous and categorical types
  ...
  # Applying various transformations to selected variables
  ...
  ## Datasets
  # Dataset after variable mapping
  ...
  if(input$id!="ID") dta %<>% rename_(ID=input$id)
  if(input$time!="TIME") dta %<>% rename_(TIME=input$time)             
  if(input$dv!="DV") dta %<>% rename_(DV=input$dv)
  if(input$sim=="TRUE") {if(input$rep!="REP") dta %<>% rename_(REP=input$rep)}
  ...
  # Dataset after variable transformation
  ...
  ############################################################################################################################
  ## Explore tab  
  dta_pk <- reactive({demo <- dta() %>% filter(!is.na(DV)) %>% group_by(UNIQUE) %>% slice(1) 
  ...
  plot <- reactive({if(is.null(input$file)) return(NULL)
    ...
    if(input$line_type=="REFERENCE" & !is.null(input$xref)) {
      p <- p + geom_vline(xintercept=as.numeric(unlist(str_split(input$xref,","))),size=rel(2),linetype="dashed",color="darkblue",alpha=0.6)
    ...
    if(input$line_type=="REFERENCE" & !is.null(input$yref)) {
      p <- p + geom_hline(yintercept=as.numeric(unlist(str_split(input$yref,","))),size=rel(2),linetype="dashed",color="darkred",alpha=0.6)
  ...
  output$timeptslider <-  renderUI({if(is.null(input$file)) return(NULL)
    tipify(sliderInput("timeptrange","Number of timepoints", 
                       min=floor(length(unique(dta()$TIME))*0.25),
                       max=ceiling(length(unique(dta()$TIME))*0.75), 
                       step=1, round=T,
                       value=c(ceiling(length(unique(dta()$TIME))*0.25),
                               floor(length(unique(dta()$TIME))*0.75))),
  ...
  output$cmaxslider <-  renderUI({if(is.null(input$file)) return(NULL)
    tipify(sliderInput("cmaxrange","Mean Cmax Percent Error Range", round=T, value=-10,
  ...
  output$indcmaxslider <- renderUI({if(is.null(input$file)) return(NULL)
    tipify(sliderInput("indcmaxrange","Cmax Percent Error Range", round=T, value=-20,
  ...
  timept.min <- floor(length(time.list)*0.25); timept.max <- ceiling(length(time.list)*0.75)
